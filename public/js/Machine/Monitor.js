                    ...
                    allowEnterKey: false
                        ...
                        return new Promise(function(resolve) {
                            $.ajax({
                                ...
                                .done(function(data) {
                                    if (data.done == 'success') {
                                        swal('清除成功!', '會員餘額' + data.credit, 'success');
                                    } else
                                        swal('I have no idea', '', 'error');
                                    RefreshStatus();
                                })
...
function VerificationCode(id) {
    $.ajax({
        url: 'Machine/Monitor/GetVerificationCode',
        type: 'POST',
        async: false,
        data: {
            id: id
        ...
        success: function(data) {
            if (data.CurPlayer == 0) {
                ...
                swal('驗證碼：' + data.VerificationCode);
            }
        },
    });
}
...
$(document).ready(function() {
    ...
    $('.machineCard').on('show.bs.tooltip', function() {
        $.ajax({
            ...
            success: function(data) {
                if (data.Name != null) {
                    $('.machineCardTooltip').attr('data-original-title', '使用者：' + data.Name.toString() + '<br>餘額：' + data.CurCredit.toLocaleString("en-US"));
                } else {
